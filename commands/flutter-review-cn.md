...
4. **完整审查**：应用完整的审查检查清单
5. **报告结果**：按严重程度分组输出问题并提供修复指导

## 前置条件

运行 `/flutter-review` 之前，请确保：
1. **构建通过** — 先运行 `/flutter-build`；对损坏的代码进行审查是不完整的
2. **测试通过** — 运行 `/flutter-test` 确认没有回归问题
3. **没有合并冲突** — 解决所有冲突，使 diff 仅反映有意的变更
4. **`flutter analyze` 无警告** — 在审查前修复分析器警告
...
状态管理: Riverpod (从 pubspec.yaml 检测)
架构: feature-first
...
[HIGH] 异步间隙后使用 BuildContext 没有 mounted 检查
文件: lib/features/auth/presentation/login_page.dart:67
问题: `context.go('/home')` 在 `await auth.login(...)` 之后调用，没有 `mounted` 检查。
修复: 在 await 后的任何导航前添加 `if (!context.mounted) return;` (Flutter 3.7+)。

[HIGH] 未处理 AsyncValue error 状态
文件: lib/features/auth/presentation/login_page.dart:42
问题: `ref.watch(authProvider)` 在 loading/data 上切换，但没有 `error` 分支。
修复: 在 switch 表达式或 `when()` 调用中添加 error case，以显示面向用户的错误消息。
...
- **阻止**: 任何 CRITICAL 或 HIGH 问题必须在合并前修复

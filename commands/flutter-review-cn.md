...
1. **构建通过** — 先运行 `/flutter-build`；对损坏代码的审查是不完整的
2. **测试通过** — 运行 `/flutter-test` 确认没有回归
3. **无合并冲突** — 解决所有冲突，以便 diff 仅反映有意的变更
4. **`flutter analyze` 干净** — 审查前修复分析器警告
...
[HIGH] 异步间隙后使用 BuildContext 未检查 mounted
文件: lib/features/auth/presentation/login_page.dart:67
问题: `await auth.login(...)` 后调用 `context.go('/home')`，未进行 `mounted` 检查。
修复: 在 await 后的任何导航前添加 `if (!context.mounted) return;`（Flutter 3.7+）。

[HIGH] AsyncValue 错误状态未处理
文件: lib/features/auth/presentation/login_page.dart:42
问题: `ref.watch(authProvider)` 的 switch 包含 loading/data 但没有 `error` 分支。

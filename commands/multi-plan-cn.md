...
- 10 分钟后仍未完成，继续用 `TaskOutput` 轮询，**绝不要终止进程**
...
4. **Task (Explore agent)**: 进行深度探索时，使用 `Task` 配合 `subagent_type: "Explore"` 在代码库中搜索
...
为降低 Claude 合成计划中的遗漏风险，可以并行让两个模型输出"计划草案" (仍 **不允许** 修改文件):

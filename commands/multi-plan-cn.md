...
- **语言协议**：与工具/模型交互时使用 **English**，与用户沟通时使用其语言
...
- 如因超时跳过等待，**必须调用** `AskUserQuestion` 询问用户是继续等待还是终止任务
...
**如果 ace-tool MCP 不可用**，使用 Claude Code 内置工具作为 fallback：
1. **Glob**：按模式查找相关文件（如 `Glob("**/*.ts")`、`Glob("src/**/*.py")`）
2. **Grep**：搜索关键符号、函数名、类定义（如 `Grep("className|functionName")`）
3. **Read**：读取发现的文件以收集完整上下文
4. **Task (Explore agent)**：如需深入探索，使用 `Task` 配合 `subagent_type: "Explore"` 在整个代码库中搜索
...
为降低 Claude 合成计划中的遗漏风险，可并行让两个模型输出"计划草稿"（仍然 **不允许** 修改文件）：
...
1. **仅规划，不实施** — 此命令不执行任何代码变更
2. **无是/否提示** — 仅呈现计划，让用户决定下一步

...
**如果 ace-tool MCP 不可用**: 跳过此步骤，后续所有阶段直接使用原始 `$ARGUMENTS`。
...
4. **Task (Explore agent)**: 如需深度探索，使用 `Task` 配合 `subagent_type: "Explore"` 在整个代码库中搜索
...
为降低 Claude 合成计划中遗漏的风险，可以并行让两个模型输出"计划草案"（仍然**不允许**修改文件）：

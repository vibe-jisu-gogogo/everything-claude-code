...
等待增强后的提示词，**将原始 $ARGUMENTS 替换为增强结果**用于后续所有阶段。
...
4. **Task (Explore agent)**: 进行更深入的探索，使用 `Task` 配合 `subagent_type: "Explore"` 在代码库中搜索

#### 1.3 完整性检查

- 必须获取相关类、函数、变量的**完整定义和签名**
- 如果上下文不足，触发**递归检索**
- 优先输出：入口文件 + 行号 + 关键符号名；仅在需要解决歧义时添加最少的代码片段
...
为降低 Claude 合成计划中遗漏的风险，可以并行让两个模型输出"计划草稿"（仍**不允许**修改文件）：
...
使用 `TaskOutput` 等待两个模型的完整结果，记录他们建议的关键差异。

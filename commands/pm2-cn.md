...
1. 检查PM2（如果缺失则通过`npm install -g pm2`安装）
...
| CRA | package.json中的react-scripts | 3000 |
...
3. **Python**: Node.js包装器脚本 + `windowsHide: true`
4. **打开新窗口**: `start wt.exe -d "{path}" pwsh -NoExit -c "command"`
5. **最小内容**: 每个命令文件仅包含1-2行描述 + bash代码块
6. **直接执行**: 无需AI解析，直接运行bash命令
...
生成文件后，将PM2部分追加到项目的`CLAUDE.md`（如果不存在则创建）：
...
**终端命令:**
```bash
pm2 start ecosystem.config.cjs   # 首次使用
pm2 start all                    # 首次之后

...
管理 Claude Code 会话历史 - 列出、加载、设置别名和编辑存储在 `~/.claude/session-data/` 中的会话，同时支持从 `~/.claude/sessions/` 读取旧版会话。
...
当你需要 swarm 的操作上下文时使用 `/sessions info`：分支、worktree 路径和会话新旧程度。
...
/sessions load 2026-02-01             # 按日期（适用于无ID会话）
...
- 会话文件在头部持久存储 `Project`、`Branch` 和 `Worktree` 信息，因此 `/sessions info` 可以区分并行的 tmux/worktree 运行。
- 对于指挥中心风格的监控，可以组合使用 `/sessions info`、`git diff --stat` 和 `scripts/hooks/cost-tracker.js` 输出的成本指标。

## 参数

$ARGUMENTS:
- `list [options]` - 列出会话
  - `--limit <n>` - 最多显示的会话数量（默认：50）
...
- 会话以 markdown 文件形式存储在 `~/.claude/session-data/` 中，同时支持从 `~/.claude/sessions/` 读取旧版会话

...
**会话复用**: 各调用返回`SESSION_ID: xxx`，后续阶段使用`resume xxx`子命令(注意: 是`resume`，不是`--resume`)。
...
**重要**:
- 必须指定`timeout: 600000`。未指定时会因默认30秒发生提前超时。
- 10分钟后仍未完成时，用`TaskOutput`继续轮询，**不强制终止进程**。
- 因超时跳过等待时，**必须调用`AskUserQuestion`询问用户是否继续等待或强制终止任务。不直接强制终止。**
...
3. 各阶段完成后要求用户确认。
4. 评分 < 7或用户不认可时强制停止。
5. 必要时使用`AskUserQuestion`工具与用户交互(例: 确认/选择/认可)。
...
1. **提示增强**(ace-tool MCP可用时): 调用`mcp__ace-tool__enhance_prompt`，**后续所有Codex/Gemini调用用增强结果替换原始$ARGUMENTS**。不可用时直接使用`$ARGUMENTS`。
2. **上下文获取**(ace-tool MCP可用时): 调用`mcp__ace-tool__search_context`。不可用时使用内置工具: `Glob`搜索文件、`Grep`搜索符号、`Read`收集上下文、`Task`(Explore代理)深度探索。
3. **需求完整性评分**(0-10):

...
- 开发任务: $ARGUMENTS
- 具备质量门的结构化6阶段工作流
- 多模型联动: Codex（后端） + Gemini（前端） + Claude（协调）
- MCP服务集成（ace-tool，可选）强化功能

## 职责

你作为**协调者**，调度多模型协作系统（调研 → 创意生成 → 计划 → 实施 → 优化 → 审查）。面向经验丰富的开发者，简洁专业地进行沟通。
...
**重要**:
- 必须指定`timeout: 600000`。未指定时将以默认30秒提前超时。
...
1. **提示词增强**（ace-tool MCP可用时）: 调用`mcp__ace-tool__enhance_prompt`，**将后续所有Codex/Gemini调用的$ARGUMENTS替换为增强结果**。不可用时直接使用`$ARGUMENTS`。
...
- Gemini: 使用分析器提示词，输出UI可行性，方案，UX评估

...
ROLE_FILE: <角色提示词路径>
<TASK>
Requirement: <增强后需求(未增强时为$ARGUMENTS)>
...
1. **提示词增强**(ace-tool MCP可用时): 调用`mcp__ace-tool__enhance_prompt`，**后续所有Codex/Gemini调用使用增强结果替换原始$ARGUMENTS**。不可用时直接使用`$ARGUMENTS`。
...
1. 阶段顺序不可跳过(除非用户明确指示)
2. 外部模型**对文件系统写入权限为零**，所有变更由Claude执行
3. 分数 < 7或用户不认可时**强制停止**

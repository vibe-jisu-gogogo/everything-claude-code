...
**重要**：
- 必须指定 \`timeout: 600000\`。不指定则默认 30 秒会导致提前超时。
...
2. **获取上下文**（ace-tool MCP 可用时）：调用 \`mcp__ace-tool__search_context\`。不可用时使用内置工具：\`Glob\` 查找文件、\`Grep\` 搜索符号、\`Read\` 收集上下文、\`Task\`（Explore 代理）进行更深入的探索。
3. **需求完整性评分**（0-10）：

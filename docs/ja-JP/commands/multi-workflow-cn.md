...
- 通过 MCP 服务集成（ace-tool，可选）强化功能

## 角色

你作为**编排者**，协调多模型协同系统（调查 → 创意生成 → 计划 → 实施 → 优化 → 审查）。面向经验丰富的开发者简洁且专业地沟通。

**联动模型**：
- **ace-tool MCP**（可选）– 代码获取 + 提示词强化
...
ROLE_FILE: <角色提示词路径>
<TASK>
Requirement: <强化后的需求（未强化时使用 $ARGUMENTS）>
...
**模型参数注意事项**：
- `{{GEMINI_MODEL_FLAG}}`: 当使用 `--backend gemini` 时，替换为 `--gemini-model gemini-3-pro-preview`（注意尾部空格）；codex 时使用空字符串
...
**重要**：
- 必须指定 `timeout: 600000`。如果不指定，会在默认的 30 秒时发生过早超时。
- 10 分钟后仍未完成时，使用 `TaskOutput` 继续轮询，**不强制终止进程**。
- 因超时跳过等待时，**必须调用 `AskUserQuestion` 询问用户是否继续等待或强制终止任务。不要直接强制终止。**
...
1. **提示词强化**（ace-tool MCP 可用时）：调用 `mcp__ace-tool__enhance_prompt`，**后续所有 Codex/Gemini 调用使用强化结果替换原始 $ARGUMENTS**。不可用时直接使用 `$ARGUMENTS`。
2. **上下文获取**（ace-tool MCP 可用时）：调用 `mcp__ace-tool__search_context`。不可用时使用内置工具：`Glob` 进行文件搜索、`Grep` 进行符号搜索、`Read` 收集上下文、`Task`（Explore agent）进行更深入的探索。
3. **需求完整性评分**（0-10）：
   - 目标明确性（0-3）、期望结果（0-3）、范围边界（0-2）、约束条件（0-2）
   - ≥7：继续 | <7：停止，询问澄清问题
...
**Claude 整合**：采用 Codex 的后端计划 + Gemini 的前端计划，经用户批准后保存到 `.claude/plan/task-name.md`。
...
**并行调用**：
- Codex：使用审查者提示词，聚焦安全、性能、错误处理
- Gemini：使用审查者提示词，聚焦可访问性、设计一致性
...
2. 外部模型**完全没有文件系统写入权限**，所有变更由 Claude 执行
3. 评分 < 7 或用户不批准时**强制停止**

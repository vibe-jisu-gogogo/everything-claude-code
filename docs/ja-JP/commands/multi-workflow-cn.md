...
**重要**：
- 必须指定 `timeout: 600000`，否则默认 30 秒会发生超时。
- 10 分钟后仍未完成时，通过 `TaskOutput` 继续轮询，**不强制终止进程**。
- 因超时跳过等待时，**必须调用 `AskUserQuestion` 询问用户是否继续等待或强制终止任务，不得直接终止**。
...
1. **提示词增强**（ace-tool MCP 可用时）：调用 `mcp__ace-tool__enhance_prompt`，**后续所有 Codex/Gemini 调用使用增强结果替换原始 $ARGUMENTS**。不可用时直接使用 `$ARGUMENTS`。
...
1. 阶段顺序不可跳过（除非用户明确指示）
2. 外部模型**零文件系统写入权限**，所有变更由 Claude 执行
3. 评分 < 7 或用户不批准时**强制停止**

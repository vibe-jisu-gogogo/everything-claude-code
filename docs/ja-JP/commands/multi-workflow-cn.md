...
多模型协作开发工作流（调研 → 创意生成 → 计划 → 实现 → 优化 → 评审），智能路由：前端 → Gemini、后端 → Codex。
...
- 通过 MCP 服务集成（ace-tool、可选）实现功能增强

## 角色

你作为**编排者**，协调多模型协作系统（调研 → 创意生成 → 计划 → 实现 → 优化 → 评审）。面向经验丰富的开发者进行简洁专业的沟通。
...
**重要**：
- 必须指定 `timeout: 600000`。否则默认 30 秒会发生过早超时。
- 10 分钟后仍未完成的情况下，继续用 `TaskOutput` 轮询，**不强制终止进程**。
- 因超时而跳过等待的情况下，**必须调用 `AskUserQuestion` 询问用户是继续等待还是强制终止任务。不要直接终止。**
...
3. 各阶段完成后请求用户确认。
4. 分数 < 7 或用户不认可时强制停止。
5. 根据需要使用 `AskUserQuestion` 工具与用户交互（例如：确认/选择/认可）。
...
1. **Prompt 增强**（ace-tool MCP 可用时）：调用 `mcp__ace-tool__enhance_prompt`，**为后续所有 Codex/Gemini 调用将原始 $ARGUMENTS 替换为增强结果**。不可用时直接使用 `$ARGUMENTS`。
2. **上下文获取**（ace-tool MCP 可用时）：调用 `mcp__ace-tool__search_context`。不可用时使用内置工具：用 `Glob` 搜索文件、用 `Grep` 搜索符号、用 `Read` 收集上下文、用 `Task`（Explore agent）进行更深入探索。
3. **需求完整性分数**（0-10）：
...
- Codex：使用分析器 Prompt，输出技术可行性、方案、风险
- Gemini：使用分析器 Prompt，输出 UI 可行性、方案、UX 评估

用 `TaskOutput` 等待结果。保存 **SESSION_ID**（`CODEX_SESSION` 和 `GEMINI_SESSION`）。

**请遵循上述「多模型调用规格」的「重要」指示**

整合双方分析，输出方案比较（至少 2 个选项），等待用户选择。
...
**Claude 整合**：采纳 Codex 的后端计划 + Gemini 的前端计划，用户确认后保存到 `.claude/plan/task-name.md`。
...
1. 阶段顺序不可跳过（除非用户明确指示）
2. 外部模型**对文件系统写入访问为零**，所有变更由 Claude 执行
3. 分数 < 7 或用户不认可时**强制停止**

...
多模型协同开发工作流（调查 → 创意生成 → 计划 → 实现 → 优化 → 审查），智能路由：前端 → Gemini，后端 → Codex。

结构化开发工作流，配备质量门、MCP服务和多模型协作。
...
作为**编排器**，协调多模型协作系统（调查 → 创意生成 → 计划 → 实现 → 优化 → 审查）。为经验丰富的开发者提供简洁专业的沟通。
...
**模型参数注意事项**：
- `{{GEMINI_MODEL_FLAG}}`：使用`--backend gemini`时替换为`--gemini-model gemini-3-pro-preview`（注意末尾空格）；使用codex时为空字符串
...
**重要**：
- 必须指定`timeout: 600000`。不指定的话默认30秒会导致提前超时。
- 10分钟后仍未完成的话，继续用`TaskOutput`轮询，**不强制终止进程**。
- 因超时跳过等待的话，**必须调用`AskUserQuestion`询问用户是继续等待还是强制终止任务，不要直接强制终止**。
...
1. 阶段顺序不可跳过（除非用户明确指示）
2. 外部模型**对文件系统零写入访问**，所有变更由Claude执行
3. 分数 < 7或用户不批准时**强制停止**

...
- **Gemini** – 前端UI/UX、可视化设计(**前端专家、后端意见仅参考**)
...
ROLE_FILE: <角色提示词路径>
<TASK>
Requirement: <增强后需求(如未增强则为$ARGUMENTS)>
...
- 10分钟后仍未完成的话，用`TaskOutput`继续轮询，**不要强制终止进程**。
...
2. 严格按顺序执行: `Research → Ideation → Plan → Execute → Optimize → Review`。
3. 各阶段完成后请求用户确认。
4. 评分 < 7或用户不认可时强制停止。
...
1. **提示词增强**(ace-tool MCP可用时): 调用`mcp__ace-tool__enhance_prompt`，**将后续所有Codex/Gemini调用的原始$ARGUMENTS替换为增强结果**。不可用时直接使用`$ARGUMENTS`。
...
1. 不能跳过阶段顺序(除非用户明确指示)
2. 外部模型**对文件系统无写入权限**，所有变更由Claude执行
3. 评分 < 7或用户不认可时**强制停止**

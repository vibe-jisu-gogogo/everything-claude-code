...
- 10分钟后仍未完成的，继续用`TaskOutput`轮询，**不强制终止进程**。
- 超时导致等待跳过的，**必须调用`AskUserQuestion`询问用户继续等待或强制终止任务。不直接强制终止。**
...
1. **提示词增强**(ace-tool MCP可用时): 调用`mcp__ace-tool__enhance_prompt`，**为后续所有Codex/Gemini调用用增强结果替换原$ARGUMENTS**。不可用时直接使用`$ARGUMENTS`。

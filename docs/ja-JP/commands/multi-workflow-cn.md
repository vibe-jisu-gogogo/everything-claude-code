...
**重要**：
- 必须指定 \`timeout: 600000\`，否则默认 30 秒会发生早期超时。
- 10 分钟后仍未完成时，通过 \`TaskOutput\` 继续轮询，**不要强制终止进程**。
- 因超时而跳过等待时，**必须调用 \`AskUserQuestion\` 询问用户是继续等待还是强制终止任务。不要直接终止。**
...
1. **提示增强**（ace-tool MCP 可用时）：调用 \`mcp__ace-tool__enhance_prompt\`，**用增强结果替换后续所有 Codex/Gemini 调用中的原始 $ARGUMENTS**。不可用时直接使用 \`$ARGUMENTS\`。

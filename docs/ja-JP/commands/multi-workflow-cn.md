...
**并行调用**：用`run_in_background: true`启动，用`TaskOutput`等待结果。**进入下一阶段前必须等待所有模型返回结果**。
...
1. **Prompt增强**（ace-tool MCP可用时）：调用`mcp__ace-tool__enhance_prompt`，**将后续所有Codex/Gemini调用的原始$ARGUMENTS替换为增强结果**。不可用时直接使用`$ARGUMENTS`。
2. **上下文获取**（ace-tool MCP可用时）：调用`mcp__ace-tool__search_context`。不可用时使用内置工具：`Glob`搜索文件、`Grep`搜索符号、`Read`收集上下文、`Task`（Explore代理）进行深层探索。
3. **需求完整性评分**（0-10）：
...
- Codex：使用分析器prompt，输出技术可行性、方案、风险
- Gemini：使用分析器prompt，输出UI可行性、方案、UX评估
...
1. 阶段顺序不可跳过（除非用户明确指示）
2. 外部模型**文件系统写入权限为零**，所有变更由Claude执行
3. 评分 < 7或用户不认可时**强制停止**

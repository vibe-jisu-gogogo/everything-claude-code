...
2. **上下文获取**（ace-tool MCP 可用时）：调用 \`mcp__ace-tool__search_context\`。不可用时使用内置工具：\`Glob\` 进行文件搜索、\`Grep\` 进行符号搜索、\`Read\` 收集上下文、\`Task\`（Explore agent）进行深度探索。
3. **需求完整性评分**（0-10）：
...
通过 \`TaskOutput\` 等待结果。整合评审反馈并在用户确认后执行优化。
...
1. 阶段顺序不可跳过（除非用户明确指示）
2. 外部模型对**文件系统写入权限为零**，所有变更由 Claude 执行
3. 评分 < 7 或用户不批准时**强制停止**

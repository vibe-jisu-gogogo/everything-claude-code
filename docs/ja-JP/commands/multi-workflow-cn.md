...
多模型协作开发工作流(调查 → 创意生成 → 计划 → 实现 → 优化 → 审查)，智能路由：前端 → Gemini，后端 → Codex。
...
你作为**编排者**，协调多模型协作系统(调查 → 创意生成 → 计划 → 实现 → 优化 → 审查)。为经验丰富的开发者提供简洁专业的沟通。
...
ROLE_FILE: <角色提示路径>
<TASK>
Requirement: <强化的需求(未强化时为$ARGUMENTS)>
...
**重要**:
- 必须指定`timeout: 600000`。未指定时默认30秒会导致提前超时。
...
1. **提示强化**(ace-tool MCP可用时): 调用`mcp__ace-tool__enhance_prompt`，**为后续所有Codex/Gemini调用将原始$ARGUMENTS替换为强化结果**。不可用时直接使用`$ARGUMENTS`。
2. **上下文获取**(ace-tool MCP可用时): 调用`mcp__ace-tool__search_context`。不可用时使用内置工具: `Glob`搜索文件、`Grep`搜索符号、`Read`收集上下文、`Task`(Explore代理)深入探索。
3. **需求完整性得分**(0-10):

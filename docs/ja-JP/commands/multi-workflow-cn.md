...
**重要**:
- 必须指定`timeout: 600000`。如不指定，默认30秒会发生超时。
- 10分钟后仍未完成时，继续使用`TaskOutput`轮询，**不强制终止进程**。
- 由于超时跳过等待时，**必须调用`AskUserQuestion`询问用户是继续等待还是强制终止任务。不得直接终止**。
...
2. **上下文获取**（ace-tool MCP可用时）：调用`mcp__ace-tool__search_context`。不可用时使用内置工具：`Glob`进行文件检索、`Grep`进行符号检索、`Read`进行上下文收集、`Task`（Explore代理）进行深度探索。
3. **需求完整性分数**（0-10）：

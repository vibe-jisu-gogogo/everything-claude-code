...
多模型协同开发工作流（调研 → 创意产生 → 计划 → 实施 → 优化 → 评审），智能路由：前端 → Gemini，后端 → Codex。
...
你作为**编排者**，协调多模型协同系统（调研 → 创意产生 → 计划 → 实施 → 优化 → 评审）。为经验丰富的开发者提供简洁专业的沟通。
...
**重要**:
- 必须指定 `timeout: 600000`。不指定则默认 30 秒会发生早期超时。
- 10 分钟后仍未完成时，用 `TaskOutput` 继续轮询，**不要强制终止进程**。
...
1. **提示词增强**（ace-tool MCP 可用时）: 调用 `mcp__ace-tool__enhance_prompt`，**将后续所有 Codex/Gemini 调用的原始 $ARGUMENTS 替换为增强结果**。不可用时直接使用 `$ARGUMENTS`。
...
**Claude 整合**: 采用 Codex 的后端计划 + Gemini 的前端计划，用户认可后保存到 `.claude/plan/task-name.md`。

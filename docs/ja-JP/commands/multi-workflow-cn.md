...
**重要**：
- 必须指定 `timeout: 600000`。未指定时默认 30 秒会发生过早超时。
- 10 分钟后仍未完成时，用 `TaskOutput` 继续轮询，**不强制终止进程**。
...
1. **Prompt 增强**（ace-tool MCP 可用时）：调用 `mcp__ace-tool__enhance_prompt`，**为后续所有 Codex/Gemini 调用用增强结果替换原始 $ARGUMENTS**。不可用时直接使用 `$ARGUMENTS`。
...
整合两者分析，输出解决方案对比（至少 2 个选项），等待用户选择。
...
**并行调用**：
- Codex：使用评审者 prompt，聚焦安全、性能、错误处理

...
多模型协作开发工作流程(调查 → 创意产生 → 计划 → 实施 → 优化 → 审查)，智能路由: 前端 → Gemini，后端 → Codex。

带有质量门、MCP服务、多模型协作的结构化开发工作流程。
...
- MCP服务集成(ace-tool、可选)功能增强

## 角色

你作为**编排器**，协调多模型协作系统(调查 → 创意产生 → 计划 → 实施 → 优化 → 审查)。面向经验丰富的开发者，简洁专业地进行沟通。
...
ROLE_FILE: <角色提示路径>
<TASK>
Requirement: <增强的需求(或未增强则为$ARGUMENTS)>
...
**会话重用**: 每次调用返回`SESSION_ID: xxx`，后续阶段使用`resume xxx`子命令(注意: 是`resume`而非`--resume`)。
...
**重要**:
- 必须指定`timeout: 600000`，否则默认30秒会导致早期超时。
- 10分钟后仍未完成时，继续用`TaskOutput`轮询，**不强制终止进程**。
- 因超时而跳过等待时，**必须调用`AskUserQuestion`询问用户是继续等待还是强制终止任务。不直接终止。**
...
1. **提示增强**(ace-tool MCP可用时): 调用`mcp__ace-tool__enhance_prompt`，**将后续所有Codex/Gemini调用的原始$ARGUMENTS替换为增强结果**。不可用时直接使用`$ARGUMENTS`。
...
**Claude整合**: 采用Codex的后端计划 + Gemini的前端计划，用户认可后保存到`.claude/plan/task-name.md`。
...
**并行调用**:
- Codex: 审查员提示，聚焦安全、性能、错误处理
- Gemini: 审查员提示，聚焦可访问性、设计一致性

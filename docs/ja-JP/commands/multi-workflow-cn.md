...
多模型协同开发工作流(调查 → 创意产生 → 计划 → 实现 → 优化 → 评审)、智能路由: 前端 → Gemini、后端 → Codex。

具备质量关卡、MCP服务、多模型协同的结构化开发工作流。
...
- MCP服务集成(ace-tool、可选)功能强化

## 角色

你作为**编排者**，协调多模型协同系统(调查 → 创意产生 → 计划 → 实现 → 优化 → 评审)。为经验丰富的开发者提供简洁专业的沟通。
...
ROLE_FILE: <角色提示词路径>
<TASK>
Requirement: <强化后的要求(未强化时为$ARGUMENTS)>
...
**会话复用**: 每次调用返回`SESSION_ID: xxx`，后续阶段使用`resume xxx`子命令(注意: `resume`、不是`--resume`)。

**并行调用**: 以`run_in_background: true`开始，使用`TaskOutput`等待结果。**进入下一阶段前必须等待所有模型返回结果**。
...
**重要**:
- 必须指定`timeout: 600000`。未指定时会因默认30秒而发生早期超时。
...
1. **提示词强化**(ace-tool MCP可用时): 调用`mcp__ace-tool__enhance_prompt`，**为后续所有Codex/Gemini调用将原始$ARGUMENTS替换为强化结果**。不可用时直接使用`$ARGUMENTS`。
2. **上下文获取**(ace-tool MCP可用时): 调用`mcp__ace-tool__search_context`。不可用时使用内置工具: `Glob`搜索文件、`Grep`搜索符号、`Read`收集上下文、`Task`(Explore代理)进行更深度探索。
3. **要求完整性评分**(0-10):
   - 目标明确性(0-3)、期望结果(0-3)、范围边界(0-2)、制约条件(0-2)
   - ≥7: 继续 | <7: 停止、询问明确化问题
...
2. 外部模型**文件系统写入访问为零**、所有变更由Claude执行
3. 评分 < 7或用户不认可时**强制停止**

...
- 通过 MCP 服务集成(ace-tool、可选) 进行功能强化

## 角色

你作为**编排器**，协调多模型协同系统(调研 → 创意生成 → 计划 → 实现 → 优化 → 评审)。面向经验丰富的开发者，沟通简洁且专业。
...
- 10 分钟后仍未完成时，继续使用 `TaskOutput` 轮询，**不强制终止进程**。
- 因超时导致等待被跳过时，**必须调用 `AskUserQuestion` 询问用户是否继续等待或强制终止任务，不得直接终止。**
...
1. **提示词强化**(ace-tool MCP 可用时): 调用 `mcp__ace-tool__enhance_prompt`，**将后续所有 Codex/Gemini 调用中的原始 $ARGUMENTS 替换为强化结果**。不可用时直接使用 `$ARGUMENTS`。
...
1. 阶段顺序不可跳过(除非用户明确指示)
2. 外部模型**对文件系统无写入权限**，所有变更由 Claude 执行
3. 分数 < 7 或用户不认可时**强制停止**

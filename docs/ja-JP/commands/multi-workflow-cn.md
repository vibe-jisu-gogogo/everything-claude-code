...
多模型协同开发工作流（调查 → 创意生成 → 计划 → 实施 → 优化 → 评审），智能路由：前端 → Gemini，后端 → Codex。
...
你作为**编排器**，协调多模型协作系统（调查 → 创意生成 → 计划 → 实施 → 优化 → 评审）。针对经验丰富的开发者进行简洁且专业的沟通。

**协作模型**：
- **ace-tool MCP**（可选）– 代码检索 + Prompt增强
...
1. **Prompt增强**（ace-tool MCP可用时）：调用`mcp__ace-tool__enhance_prompt`，**将后续所有Codex/Gemini调用的原$ARGUMENTS替换为增强结果**。不可用时使用原`$ARGUMENTS`。
2. **上下文获取**（ace-tool MCP可用时）：调用`mcp__ace-tool__search_context`。不可用时使用内置工具：`Glob`检索文件、`Grep`检索符号、`Read`收集上下文、`Task`（Explore代理）进行深度探索。
3. **需求完整性评分**（0-10）：
...
**并行调用**：
- Codex：使用评审者Prompt，聚焦安全、性能、错误处理
- Gemini：使用评审者Prompt，聚焦可访问性、设计一致性

使用`TaskOutput`等待结果。整合评审反馈，用户确认后实施优化。

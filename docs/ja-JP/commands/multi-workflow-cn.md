...
- 通过MCP服务集成(ace-tool、可选)进行功能强化

## 角色

你作为**编排者**，协调多模型协同系统(调查 → 创意生成 → 计划 → 实施 → 优化 → 审查)。面向经验丰富的开发者进行简洁且专业的沟通。
...
**重要**:
- 必须指定`timeout: 600000`。否则默认30秒会发生早期超时。
- 10分钟后仍未完成时，继续用`TaskOutput`轮询，**不强制终止进程**。
- 因超时而跳过等待时，**必须调用`AskUserQuestion`询问用户是否继续等待或强制终止任务。不直接强制终止。**
...
3. 各阶段完成后要求用户确认。
4. 评分 < 7或用户不批准时强制停止。
5. 根据需要使用`AskUserQuestion`工具与用户交互(例: 确认/选择/批准)。

---
name: eval-harness
description: 为 Claude Code 会话实现评估驱动开发 (EDD) 原则的官方评估框架
origin: ECC
...
为 Claude Code 会话实现评估驱动开发 (EDD) 原则的官方评估框架。

## 何时使用

- 为 AI 辅助工作流建立评估驱动开发 (EDD) 时
- 定义 Claude Code 任务完成的通过/失败标准时
- 使用 pass@k 指标衡量代理可靠性时
- 为提示词或代理变更创建回归测试套件时
- 在模型版本之间对代理性能进行基准测试时
...
2. **经常运行评估** - 及早发现回归
3. **跟踪随时间变化的 pass@k** - 监控可靠性趋势
4. **在可能的情况下使用代码评分器** - 确定性 > 概率性
5. **针对安全性使用人工审核** - 不要完全自动化安全检查
6. **保持评估快速** - 缓慢的评估不会被执行
7. **与代码一起版本控制评估** - 评估是一等产出物

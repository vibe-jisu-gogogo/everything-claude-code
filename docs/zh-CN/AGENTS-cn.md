...
这是一个**生产就绪的 AI 编码插件**，提供 48 个专业代理、182 项技能、68 条命令以及自动化 hook 工作流，用于软件开发。

**版本:** 2.0.0-rc.1

## 核心原则

1. **Agent-First** — 将领域任务委托给专业智能体
2. **Test-Driven** — 先写测试再实现，要求 80%+ 覆盖率
3. **Security-First** — 绝不妥协安全；验证所有输入
4. **Immutability** — 总是创建新对象，永不修改现有对象
5. **Plan Before Execute** — 在编写代码前规划复杂功能
...
- Harness 配置可靠性及成本 → **harness-optimizer**
...
**Immutability（关键）：** 总是创建新对象，永不修改。返回带有更改的新副本。
...
1. **Plan** — 使用 planner 代理，识别依赖关系和风险，分阶段推进
2. **TDD** — 使用 tdd-guide 代理，先写测试，再实现和重构
3. **Review** — 立即使用代码评审代理，解决 CRITICAL/HIGH 级别的问题
4. **在适当位置记录知识**
   - 个人调试笔记、偏好和临时上下文 → auto memory
   - 团队/项目知识（架构决策、API 变更、操作手册）→ 项目现有文档结构
   - 如果当前任务已生成相关文档或代码注释，请勿在其他地方重复相同信息
   - 如果没有明显的项目文档位置，在创建新的顶层文件前先询问
5. **Commit** — 采用约定式提交格式，提供全面的 PR 摘要

## 工作流表面策略

- `skills/` 是规范的工作流表面。
- 新的工作流贡献应首先落在 `skills/` 中。
- `commands/` 是遗留的斜杠入口兼容性表面，只有在迁移或跨工具奇偶校验仍需要垫片时，才应添加或更新。
...
**Repository 模式：** 将数据访问封装在标准接口（findAll, findById, create, update, delete）后面。业务逻辑依赖于抽象接口，而不是存储机制。

**Skeleton 项目：** 搜索经过实战检验的模板，使用并行智能体（安全性、可扩展性、相关性）进行评估，克隆最佳匹配，在已验证的结构内迭代。
...
`commands/` 保留在仓库中以保持兼容性，但长期方向是 skills-first。

...
- 新增 `docs/architecture/cross-harness.md`，并补充 ECC/Hermes 边界的 regression 覆盖。
...
- `brand-voice` — 规范的源派生写作风格系统。
- `social-graph-ranker` — 加权的 warm-intro 图形排名原语。
- `connections-optimizer` — 基于图形排名的网络修剪/添加工作流。
...
- `cargo build --manifest-path ecc2/Cargo.toml` 在仓库基准上通过。
- `ecc-tui` 当前暴露 `dashboard`、`start`、`sessions`、`status`、`stop`、`resume` 和 `daemon`。
- alpha 版本真实可用，可用于本地实验，但更广泛的 control-plane 路线图仍不完整，不应视为 GA。
...
- Selective install 架构，采用 manifest-driven 流水线和 SQLite 状态存储。
- 语言覆盖范围扩展至 10 多个生态，新增 6 个代理和语言特定规则。
- Observer 可靠性增强，包括内存限制、沙箱修复和 5 层循环防护。
- 自我改进的技能基础，支持技能演进和会话适配器。

### 新代理

- `typescript-reviewer` — TypeScript/JavaScript 代码审查专家 (#647)
- `pytorch-build-resolver` — PyTorch runtime、CUDA 及训练错误解决 (#549)
...
- `bun-runtime` — Bun runtime 模式 (#529)
...
- `ai-regression-testing` — AI regression 测试工作流 (#433)
...
- Selective install 架构，支持 manifest 解析 (`install-plan.js`、`install-apply.js`) (#509, #512)
...
- Observer 内存爆炸问题，通过限制、重入防护和尾部采样解决 (#536)
- Observer 沙箱访问修复，用于 Haiku 调用 (#661)
- Worktree 项目 ID 不匹配修复 (#665)
- Observer 延迟启动逻辑 (#508)
- Observer 5 层循环预防防护 (#399)
...
- Hook runtime 现在支持基于配置文件的控制和针对性的 Hook 禁用。

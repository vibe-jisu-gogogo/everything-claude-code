...
description: 将项目/全局范围的 instinct 导出到文件
...
/instinct-export --min-confidence 0.7      # 仅导出高置信度的 instinct
...
4. 将 YAML 格式的导出写入文件（如果未提供输出路径则写入 stdout）
...
- `--domain <name>`: 仅导出指定的 domain
- `--min-confidence <n>`: 最小 confidence 阈值
